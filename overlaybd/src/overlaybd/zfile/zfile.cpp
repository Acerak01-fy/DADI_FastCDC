...
public:
    struct HeaderTrailer {
        ...
        static const uint32_t FLAG_SHIFT_FASTCDC = 6; // use FastCDC variable-length chunking
        ...
        void set_fastcdc() {
            set_flag_bit(FLAG_SHIFT_FASTCDC);
        }

        bool is_fastcdc() const {
            return get_flag_bit(FLAG_SHIFT_FASTCDC);
        }
    ...
    // Store raw (uncompressed) block sizes for variable-length blocks (FastCDC)
    std::vector<uint32_t> m_raw_block_sizes;
    ...
    public:
        ...
        BlockReader(const CompressionFile *zfile, size_t offset, size_t count)
            : m_zfile(zfile), m_offset(offset), m_original_offset(offset) /* , m_count(count)  */
        {
            m_verify = zfile->m_ht.opt.verify;
            m_block_size = zfile->m_ht.opt.block_size; //+ m_verify * sizeof(uint32_t);
            
            // For FastCDC, use raw_block_sizes to find block index
            if (zfile->m_ht.is_fastcdc() && !zfile->m_raw_block_sizes.empty()) {
                // Find which block contains the starting offset
                size_t cumulative_offset = 0;
                m_begin_idx = 0;
                for (size_t i = 0; i < zfile->m_raw_block_sizes.size(); i++) {
                    if (cumulative_offset + zfile->m_raw_block_sizes[i] > offset) {
                        m_begin_idx = i;
                        break;
                    }
                    cumulative_offset += zfile->m_raw_block_sizes[i];
                }
                m_idx = m_begin_idx;
                m_original_begin_idx = m_begin_idx;  // Save the true first block index
                m_end = offset + count - 1;
                
                // Find ending block index
                cumulative_offset = 0;
                m_end_idx = zfile->m_raw_block_sizes.size();
                for (size_t i = 0; i < zfile->m_raw_block_sizes.size(); i++) {
                    cumulative_offset += zfile->m_raw_block_sizes[i];
                    if (cumulative_offset > (size_t)m_end) {
                        m_end_idx = i + 1;
                        break;
                    }
                }
        ...
        __attribute__((always_inline)) size_t get_inblock_offset(size_t offset, size_t blk_idx) {
            if (m_zfile->m_ht.is_fastcdc() && !m_zfile->m_raw_block_sizes.empty()) {
                // Calculate offset from the beginning of the specified block
                size_t cumulative = 0;
                for (size_t i = 0; i < blk_idx; i++) {
                    cumulative += m_zfile->m_raw_block_sizes[i];
                }
                auto result = offset - cumulative;
                LOG_DEBUG("get_inblock_offset: offset=`, blk_idx=`, cumulative=`, result=`", 
                          offset, blk_idx, cumulative, result);
                return result;
            }
        ...
        __attribute__((always_inline)) size_t block_size() const {
            if (m_zfile->m_ht.is_fastcdc() && !m_zfile->m_raw_block_sizes.empty() && 
                m_idx < m_zfile->m_raw_block_sizes.size()) {
                return m_zfile->m_raw_block_sizes[m_idx];
            }
            return m_block_size;
        }
        ...
        struct iterator {
            ...
            int get_current_block() {
                ...
                if (blk_idx == m_reader->m_original_begin_idx) {
                    // Only the very first block uses m_original_offset
                    cp_begin = m_reader->get_inblock_offset(m_reader->m_original_offset, blk_idx);
                    LOG_DEBUG("First block (original): blk_idx=`, m_original_offset=`, cp_begin=`", blk_idx, m_reader->m_original_offset, cp_begin);
                    m_reader->m_offset = 0;
                } else {
                    cp_begin = 0;
                }
                ...
                if (blk_idx == m_reader->m_end_idx - 1) {
                    auto inblock_end = m_reader->get_inblock_offset(m_reader->m_end, blk_idx);
                    LOG_DEBUG("Last block: blk_idx=`, m_end=`, inblock_end=`, block_size=`", 
                              blk_idx, m_reader->m_end, inblock_end, cp_len);
                    cp_len = inblock_end + 1;
                }
                cp_len -= cp_begin;
                LOG_DEBUG("Block `: cp_begin=`, cp_len=`, compressed_size=`", blk_idx, cp_begin, cp_len, compressed_size);
                return 0;
            }
        ...
        size_t m_original_begin_idx = 0;  // Save the very first block index
        size_t m_idx = 0;
        size_t m_end_idx = 0;
        off_t m_offset = 0;
        off_t m_original_offset = 0;  // Save original offset for FastCDC
        off_t m_end = 0;
        uint8_t m_verify = 0;
    ...
    virtual ssize_t pread(void *buf, size_t count, off_t offset) override {
        ...
        for (auto &block : br) {
            ...
            // For FastCDC: check if we've already read enough data
            if (readn >= cnt) {
                LOG_DEBUG("Already read enough data, stopping: readn=`, cnt=`", readn, cnt);
                break;
            }
        ...
        again:
            ...
            // For FastCDC: calculate how many bytes we can actually use from this block
            auto bytes_to_copy = block.cp_len;
            if (readn + bytes_to_copy > (size_t)cnt) {
                bytes_to_copy = cnt - readn;
                LOG_DEBUG("Limiting block usage: cp_len=`, bytes_to_copy=`, readn=`, cnt=`", 
                          block.cp_len, bytes_to_copy, readn, cnt);
            }
            
            int dret = -1;
            if (block.cp_len == m_ht.opt.block_size && bytes_to_copy == block.cp_len) {
                // Full block, decompress directly to output buffer
                dret = m_compressor->decompress(block.buffer(), block.compressed_size,
                                                (unsigned char *)buf, m_ht.opt.block_size);
            } else {
                // Partial block or FastCDC variable size, decompress to temp buffer
                dret = m_compressor->decompress(block.buffer(), block.compressed_size, raw,
                                                m_ht.opt.block_size);
                if (dret != -1)
                    memcpy(buf, raw + block.cp_begin, bytes_to_copy);  // Use limited size
            }
            if (dret == -1) {
                if (retry--) {
                    int reload_res = block.reload();
            ...
            readn += bytes_to_copy;
            buf = (unsigned char *)buf + bytes_to_copy;
        }
        if (br.m_eno != 0) {
            LOG_ERRNO_RETURN(br.m_eno, -1, "read compressed data failed.");
        }
        // For FastCDC: ensure we don't return more than requested
        // (BlockReader may traverse multiple small blocks that exceed cnt)
        if (readn > cnt) {
            LOG_WARN("pread read more than requested: readn=`, cnt=`, clamping", readn, cnt);
            readn = cnt;
        }
        return readn;
    }
...
again:
    ...
    // Load raw block sizes for FastCDC
    if (ht.is_fastcdc()) {
        LOG_INFO("FastCDC detected, loading raw block sizes");
        auto raw_index_offset = ht.index_offset + ht.index_size * sizeof(uint32_t);
        auto raw_index_bytes = ht.index_size * sizeof(uint32_t);
        zfile->m_raw_block_sizes.resize(ht.index_size);
        auto readn = file->pread(&zfile->m_raw_block_sizes[0], raw_index_bytes, raw_index_offset);
        if (readn != (ssize_t)raw_index_bytes) {
            delete zfile;
            LOG_ERRNO_RETURN(0, nullptr, "failed to read raw block sizes");
        }
        LOG_INFO("Loaded ` raw block sizes", ht.index_size);
    }
...
int zfile_compress(IFile *file, IFile *as, const CompressArgs *args) {
    ...
    // Adjust block_size for FastCDC
    if (args->use_fastcdc) {
        opt.block_size = args->fastcdc_max_size;
    }
    ...
    if (args->use_fastcdc) {
        pht->set_fastcdc();
        LOG_INFO("FastCDC enabled in header");
    }
    LOG_INFO("write header.");
    ...
    // FastCDC initialization
    FastCDCChunker* fastcdc_chunker = nullptr;
    if (args->use_fastcdc) {
        fastcdc_chunker = new FastCDCChunker(args->fastcdc_min_size, 
                                              args->fastcdc_avg_size, 
                                              args->fastcdc_max_size);
        LOG_INFO("FastCDC enabled: min=`, avg=`, max=`", 
                 args->fastcdc_min_size, args->fastcdc_avg_size, args->fastcdc_max_size);
        // Use max_size for buffer allocation
        block_size = args->fastcdc_max_size;
    }
    DEFER(if (fastcdc_chunker) delete fastcdc_chunker);
    
    auto buf_size = block_size + BUF_SIZE;
    bool crc32_verify = opt.verify;
    std::vector<uint32_t> block_len{};       // compressed block sizes
    std::vector<uint32_t> raw_block_len{};   // uncompressed block sizes (for FastCDC variable blocks)
    ...
    while (true) {
        ...
        size_t batch_start_offset = 0;  // For FastCDC: track where current batch starts
        
        // Chunking logic: FastCDC or fixed-size
        if (args->use_fastcdc) {
            // FastCDC content-defined chunking
            size_t offset = 0;
            
            while (offset < (size_t)readn) {
                ...
                if (chunk_size == 0 || chunk_size > remaining) {
                    // Use remaining data as the last chunk
                    chunk_size = remaining;
                }
                
                LOG_DEBUG("FastCDC chunk #`: size=`, offset=`, remaining=`", n, chunk_size, offset, remaining);
                raw_chunk_len[n++] = chunk_size;
                offset += chunk_size;
                
                // Check if we need to compress this batch before continuing
                if (n >= nbatch) {
                    LOG_DEBUG("FastCDC batch full, compressing ` chunks from offset `", n, batch_start_offset);
                    LOG_DEBUG("  raw_data ptr: `, src ptr: `, chunk_len[0]: `", 
                              (void*)raw_data, (void*)(raw_data + batch_start_offset), raw_chunk_len[0]);
                    auto compress_ret = compressor->compress_batch(raw_data + batch_start_offset, 
                                                                    &(raw_chunk_len[0]), compressed_data,
                                                                    n * buf_size, &(compressed_len[0]), n);
                    if (compress_ret != 0)
                        return -1;
                    for (int i = 0; i < n; i++) {
                        LOG_DEBUG("  Compressed block `: ` -> ` bytes", i, raw_chunk_len[i], compressed_len[i]);
                    }
                    for (off_t j = 0; j < n; j++) {
                        auto write_ret = as->write(&compressed_data[j * buf_size], compressed_len[j]);
                        if (write_ret < (ssize_t)compressed_len[j]) {
                            LOG_ERRNO_RETURN(0, -1, "failed to write compressed data.");
                        }
                        if (crc32_verify) {
                            auto crc32_code = crc32c_salt(&compressed_data[j * buf_size], compressed_len[j]);
                            LOG_DEBUG("append ` bytes crc32_code", sizeof(uint32_t));
                            compressed_len[j] += sizeof(uint32_t);
                            write_ret = as->write(&crc32_code, sizeof(uint32_t));
                            if (write_ret < (ssize_t)sizeof(uint32_t)) {
                                LOG_ERRNO_RETURN(0, -1, "failed to write crc32code");
                            }
                        }
                        block_len.push_back(compressed_len[j]);
                        raw_block_len.push_back(raw_chunk_len[j]);  // Save uncompressed size
                        moffset += compressed_len[j];
                    }
                    batch_start_offset = offset;  // Next batch starts from current offset
                    n = 0;  // Reset for next batch
                }
            }
            LOG_DEBUG("FastCDC chunking done: final_chunks=`, total_size=`", n, offset);
        } else {
        ...
        // Compress remaining chunks (if any)
        if (n > 0) {
            auto src_ptr = args->use_fastcdc ? (raw_data + batch_start_offset) : raw_data;
            LOG_DEBUG("Compressing remaining ` chunks, src_ptr offset: `, batch_start_offset: `", 
                      n, (void*)src_ptr, batch_start_offset);
            for (int i = 0; i < n; i++) {
                LOG_DEBUG("  Chunk `: size=`", i, raw_chunk_len[i]);
            }
            readn = compressor->compress_batch(src_ptr, &(raw_chunk_len[0]), compressed_data,
                                             n * buf_size, &(compressed_len[0]), n);
            if (readn != 0)
                return -1;
            for (off_t j = 0; j < n; j++) {
    ...
    // Write raw block sizes (for FastCDC variable-length blocks)
    if (args->use_fastcdc) {
        ssize_t raw_index_bytes = raw_block_len.size() * sizeof(uint32_t);
        LOG_INFO("write raw block sizes (offset: `, count: `, size: `)", 
                 moffset + index_bytes, raw_block_len.size(), raw_index_bytes);
        if (as->write(&raw_block_len[0], raw_index_bytes) != raw_index_bytes) {
            LOG_ERRNO_RETURN(0, -1, "failed to write raw block sizes.");
        }
    }
...
int zfile_decompress(IFile *src, IFile *dst) {
    ...
    // For FastCDC, we need to read sequentially from offset 0 to end
    // Because we can't know the block boundaries without reading
    off_t offset = 0;
    while (offset < raw_data_size) {
        auto remaining = raw_data_size - offset;
        auto len = (ssize_t)std::min(block_size, (size_t)remaining);
        LOG_INFO("Requesting ` bytes at offset `, remaining=`", len, offset, remaining);
        auto readn = file->pread(raw_buf.get(), len, offset);
        LOG_INFO("pread returned: ` bytes (requested: `)", readn, len);
        if (readn <= 0) {
            LOG_ERROR_RETURN(0, -1, "failed to read at offset `", offset);
        }
        if (readn > (ssize_t)len) {
            LOG_ERROR("WARNING: pread returned more data than requested! readn=`, len=`", readn, len);
            readn = len;  // Clamp to prevent buffer overflow
        }
        // Only calculate CRC for the valid data size
        LOG_INFO("Got ` bytes, crc32: `", readn, HEX(crc32c_salt(raw_buf.get(), readn)).width(8));
        LOG_INFO("Writing ` bytes to output", readn);
        auto writen = dst->write(raw_buf.get(), readn);
        if (writen != readn) {
            LOG_ERROR_RETURN(0, -1, "failed to write file into dst (wrote ` of ` bytes)", writen, readn);
        }
        offset += readn;
    }
    return 0;
